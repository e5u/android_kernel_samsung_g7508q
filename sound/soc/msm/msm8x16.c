...
void msm8x16_enable_ear_micbias(bool state)
{
	...
	pr_info("%s : board rev %d =========\n",__func__, system_rev);
#if defined (CONFIG_SEC_VASTA_PROJECT) || defined(CONFIG_SEC_VASTA3G_PROJECT)
	if(system_rev >= 5)
#endif /* system_rev check end */
#if defined (CONFIG_SEC_VASTA_PROJECT) || defined(CONFIG_SEC_VASTA3G_PROJECT) \
	|| defined (CONFIG_SEC_A5_PROJECT) || defined (CONFIG_SEC_A5_EUR_PROJECT) || defined (CONFIG_SEC_A53G_EUR_PROJECT)
	{
		if (state == 1) {
			nRetVal = snd_soc_dapm_force_enable_pin(dapm, "MIC BIAS Internal2");
			pr_info("%s enable the codec  MIC BIAS Internal2 pin : %d with state :%d\n"
					, __func__, nRetVal, state);
		} else {
			nRetVal = snd_soc_dapm_disable_pin(dapm, "MIC BIAS Internal2");
			pr_info("%s disable the codec MIC BIAS Internal2 pin : %d with state :%d\n"
					, __func__, nRetVal, state);
		}
	}
#endif /* internal2 bias setting end */
	snd_soc_dapm_sync(dapm);
